...
public final class HomeCommandAdapter {
    ...
    public boolean home(Player player, String[] args) {
        ...
        plugin.daemon().ifPresentOrElse(client -> client.send(request("player.home.get", Map.of(
            "playerUuid", player.getUniqueId().toString(),
            "home", args[0]
        ))).thenAccept(response -> applyHome(player, response.body().get("raw"))),
            () -> player.sendMessage(message(player, "daemon.unavailable", Map.of())));
        return true;
    }

    private void applyHome(Player player, Object raw) {
        ...
        if (!instanceId().equals(extract(json, "serverId").orElse(""))) {
            plugin.scheduler().runPlayer(player, () -> player.sendMessage(message(player, "home.wrong-server", Map.of())));
            return;
        }
