...
import com.lkjmc.paper.ui.UiWiring;
...
public final class LkjmcPaperPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (!access.available()) {
            getLogger().warning("lkjmc daemon access unavailable: " + access.code());
        }
        this.daemon = access.client();
        ...
        var transfers = new ProfileTransferChannel(this);
        var ui = UiWiring.create(this, transfers::transfer);
        var commands = new PaperCommands(this, ui.entrypoints(), catalog, resolver);
        var docs = new DocsCommandAdapter(ui.entrypoints());
        ...
        getServer().getPluginManager().registerEvents(ui.inventoryListener(), this);
        getServer().getPluginManager().registerEvents(ui.textInput(), this);
        getServer().getPluginManager().registerEvents(new HotbarMenuListener(ui.entrypoints(), catalog, locales, token, inventorySync), this);
