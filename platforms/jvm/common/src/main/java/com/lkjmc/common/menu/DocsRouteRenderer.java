...
final class DocsRouteRenderer {
    private static final int[] ROWS = {19,20,21,22,23,24,25,28,29,30,31,32,33,34,37,38,39,40,41,42,43};
    private final DocBundle docs;
    private final MessageCatalog messages;

    DocsRouteRenderer(DocBundle docs, MessageCatalog messages) {
        this.docs = docs; this.messages = messages;
    }
    ...
    List<MenuFrame.Slot> render(MenuRoute route, Map<String, String> params, String locale,
        ...
        switch (route.dynamic().binding()) {
            case DOCS_DIRECTORY -> files(route, docs.files(), locale, session, request, revision, slots);
            case DOCS_SEARCH -> files(route, docs.search(params.getOrDefault("query", "")), locale,
                    session, request, revision, slots);
            case DOCS_FILE -> file(route, params, locale, session, request, revision, slots);
            case DOCS_LINKS -> links(route, params, locale, session, request, revision, slots);
            default -> throw new IllegalArgumentException("not a docs binding");
        }
        if (slots.isEmpty()) slots.add(inert(route, 22, "PAPER", localized(locale,
                route.dynamic().emptyNameKey() == null ? "menu.docs.search.empty" : route.dynamic().emptyNameKey()),
    ...
    private void files(MenuRoute route, List<DocFile> files, String locale, long session,
        ...
        int index = 0;
        for (var file : files) {
            var action = new MenuAction.Navigate("docs-file", Map.of("path", file.path(), "page", "0"));
            slots.add(slot(route, ROWS[index++], "BOOK", file.title(), List.of(file.path()),
                    MenuTypes.Role.NAVIGATION, action, session, request, revision));
    ...
    private void file(MenuRoute route, Map<String, String> params, String locale, long session,
        ...
        int requested;
        try { requested = Integer.parseInt(params.getOrDefault("page", "0")); }
        catch (NumberFormatException failure) { requested = 0; }
        var page = DocPaginator.page(value.get(), requested, 36);
        for (int index = 0; index < page.lines().size(); index++) {
            slots.add(inert(route, ROWS[index], "PAPER", page.lines().get(index), session, request, revision));
        }
        if (page.page() > 0) slots.add(slot(route, 46, "ARROW", localized(locale, "docs.previous"), List.of(),
                MenuTypes.Role.NAVIGATION, pageAction(value.get().path(), page.page() - 1), session, request, revision));
        if (page.page() + 1 < page.pageCount()) slots.add(slot(route, 48, "ARROW", localized(locale, "docs.next"), List.of(),
                MenuTypes.Role.NAVIGATION, pageAction(value.get().path(), page.page() + 1), session, request, revision));
        if (!value.get().links().isEmpty()) slots.add(slot(route, 51, "LECTERN", localized(locale, "docs.links"), List.of(),
                MenuTypes.Role.NAVIGATION, new MenuAction.Navigate("docs-links", Map.of("path", value.get().path())),
                session, request, revision));
    }
    ...
    private void links(MenuRoute route, Map<String, String> params, String locale, long session,
        ...
        for (var link : source.get().links()) {
            ...
            slots.add(slot(route, ROWS[index++], "PAPER", link.text(), List.of(),
                    ...
                    action, session, request, revision));
            if (index == ROWS.length) break;
        }
    }
    ...
    private MenuAction pageAction(String path, int page) {
        return new MenuAction.Navigate("docs-file", Map.of("path", path, "page", Integer.toString(page)));
    }
    private MenuFrame.Slot inert(MenuRoute route, int index, String material, String name,
        ...
        return slot(route, index, material, name, List.of(), MenuTypes.Role.INFO,
                new MenuAction.Simple(MenuTypes.ActionType.NONE), session, request, revision);
    }
    private MenuFrame.Slot slot(MenuRoute route, int index, String material, String name, List<String> lore,
                                MenuTypes.Role role, MenuAction action, long session, long request, long revision) {
        String clean = name.replaceAll("<[^>]+>", "").trim();
        return MenuFrame.slot(index, material, clean.isBlank() ? " " : clean, lore, role, action,
                route.id(), session, request, revision);
    }
