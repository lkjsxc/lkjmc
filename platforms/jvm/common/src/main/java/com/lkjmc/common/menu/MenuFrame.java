...
public record MenuFrame(String route, String title, int size, long session,
                        long request, long renderRevision, List<Slot> slots) {
    ...
    public record Metadata(String route, long session, long request,
                           long renderRevision, int slot, MenuTypes.ActionType action) {}
    ...
    public static Slot slot(int index, String material, String name, List<String> lore,
        ...
        var metadata = new Metadata(route, session, request, revision, index, action.type());
