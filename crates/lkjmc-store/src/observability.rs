...
const EVENT_NAMESPACE: Uuid = Uuid::from_u128(0xc6fc529d_1327_45d5_bfdf_b19572e0dc2e);
...
pub fn record_command<C: GenericClient>(
    ...
    upsert_operation(client, request, &event)?;
...
pub fn command_event(
    ...
    let (operation_id, correlation_id) =
        correlation_ids(request.request_id.as_str(), &request.body);
...
fn upsert_operation<C: GenericClient>(
    ...
    client.execute(
        ...
        &[&event.operation_id, &event.request_id, &event.correlation_id, &bounded(&request.command, 96),
...
fn event_id(event: &EventEnvelope) -> Uuid {
    let key = format!(
        "{}:{:?}:{:?}",
