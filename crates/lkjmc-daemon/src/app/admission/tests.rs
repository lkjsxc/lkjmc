...
#[tokio::test]
async fn explicit_long_budget_overrides_only_the_request_deadline() -> Result<(), String> {
    let admission = Admission::with_deadline(Duration::from_millis(10));
    let request = admission.try_admit().ok_or("admission missing")?;
    request
        .run_blocking_with_budget(Duration::from_millis(100), || {
            std::thread::sleep(Duration::from_millis(30))
        })
        .await
        .map_err(|_| "explicit long budget was not honored".to_string())?;
    drop(request);
    admission
        .wait_for_idle()
        .await
        .map_err(|_| "long-budget worker join failed".to_string())?;
    Ok(())
}
