...
fn cross_instance_database_process_hang() -> Result<(), String> {
    ...
    let holder = std::thread::spawn(move || {
        held_state.coordinate_runtime("held", || {
    ...
    std::thread::spawn(move || {
        let _ = peer_tx.send(start_runtime(&peer_state, "peer"));
    });
...
fn same_instance_database_process_race() -> Result<(), String> {
    ...
    let start = std::thread::spawn(move || {
        start_barrier.wait();
        let _ = start_done.send(start_runtime(&start_state, "race").map(|_| ()));
    });
    let stop_state = Arc::clone(&state);
    let stop_barrier = Arc::clone(&barrier);
    let stop = std::thread::spawn(move || {
        stop_barrier.wait();
        let _ = done_tx.send(stop_runtime(&stop_state, "race").map(|_| ()));
    });
    barrier.wait();
    for _ in 0..2 {
        done_rx
    ...
    let starts: i64 = fixture
        ...
        .get(0);
    assert!(starts <= 1);
    let observation = start_runtime(&state, "race")?;
