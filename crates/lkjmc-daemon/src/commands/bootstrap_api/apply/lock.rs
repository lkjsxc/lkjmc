...
fn acquire_with_deadline(root: &str, lock_wait: Duration, total: Duration) -> Result<ApplyGuard, String> {
    ...
    let file = OpenOptions::new().read(true).write(true).create(true).mode(0o600)
        .open(&path).map_err(|error| format!("open network apply lock: {error}"))?;
    let wait_deadline = Instant::now() + lock_wait;
    loop {
        match file.try_lock_exclusive() {
            Ok(()) => return Ok(ApplyGuard { _file: file, deadline: Instant::now() + total }),
            Err(error) if error.kind() == std::io::ErrorKind::WouldBlock && Instant::now() < wait_deadline => {
