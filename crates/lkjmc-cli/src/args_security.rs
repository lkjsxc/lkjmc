...
fn create(values: &[String]) -> Result<CliCommand, CliError> {
    ...
    while index < values.len() {
        ...
        match flag.as_str() {
            ...
            "--principal-kind" => principal_kind = value,
            "--principal-id" => principal_id = value,
            "--output-file" => output_file = value,
            "--expires-in-seconds" => {
                expiry = value
                    .parse()
                    .map_err(|_| CliError::message("expiry must be an integer"))?
            }
            "--scope" => scopes.push(value),
...
fn usage() -> &'static str {
    "usage: lkjmc security token plan|rotate|status|verify|create --surface NAME --principal-kind KIND --principal-id ID --output-file ABSOLUTE --expires-in-seconds N --scope SCOPE...|revoke --credential-id ID"
}
