...
pub fn run(socket: &str, command: SecurityCommand, json_output: bool) -> Result<(), CliError> {
    let (daemon, body) = match &command {
        ...
        SecurityCommand::Create {
            ...
            json!({"surface":surface,"principalKind":principal_kind,"principalId":principal_id,"outputFile":output_file,"expiresInSeconds":expires_in_seconds,"scopes":scopes}),
        ),
