...
"artifact-provenance-pass":[("scripts/release_inventory.py","release artifact closure differs from contract","tracked_contract_paths","image_items"),("scripts/verify-artifact-manifest.py","independently derived release closure","fullmatch"),("scripts/operations_semantic_checks.py","FIXTURE_COMMIT","LKJMC_SOURCE_COMMIT"),("scripts/run-operations-lab.py","retained artifact index is not exact")],
...
"ci-compose-retained":[(".github/workflows/verify.yml","LKJMC_SCAN_CANARY","context.tar","docker image inspect","docker image save","audit-saved-image.py","compose-config.raw","if: always() && steps.secret-scan.outcome == 'success'","Upload only safe failure marker"),("scripts/audit-saved-image.py","MAX_ARCHIVE","manifest.json","declared image layer missing","duplicate conflicting member","unreferenced saved image member","conflicting shared layer digest"),("scripts/saved_image_semantic_checks.py","images=2 layerReferences=2 layers=1","missing","duplicate","hidden.tar"),("scripts/prepare-operations-evidence.py","LIMITS","artifact-index.json"),("scripts/scan-secrets.py","credential URL","generated canary","[A-Za-z0-9._~+%-]+","(?<![A-Za-z0-9/])Bearer")],
}
