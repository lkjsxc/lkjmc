...
"artifact-provenance-pass":[("scripts/release_inventory.py","release artifact closure differs from contract","tracked_contract_paths","image_items","release provenance requires a clean Git checkout"),("scripts/build-release.sh","git worktree add","LKJMC_BUILD_NONCE","fresh built artifact","release output parent must not be group/other writable","refusing cleanup of replaced release output"),("scripts/attach-source-git.sh","git fetch","exported source differs from bundled Git object"),("scripts/verify-built-identity.py","compiled JVM identity differs","does not report a clean build"),("tests/test_release_identity.py","linked ref","ambient-substitution","source-link","compiled JVM identity differs"),("scripts/verify-artifact-manifest.py","independently derived release closure","fullmatch"),("scripts/operations_semantic_checks.py","shutil.copytree(ROOT,fixture","git','commit','-q','-m','fixture"),("scripts/run-operations-lab.py","retained artifact index is not exact")],
