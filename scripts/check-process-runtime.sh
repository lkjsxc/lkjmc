...
log=$(mktemp "${TMPDIR:-/tmp}/lkjmc-runtime.XXXXXX.log")
...
if target/debug/lkjmc --socket "$socket" instance start denied-runtime >"$out" 2>&1; then
    cat "$out"
    exit 1
fi
grep -q 'command.effect_denied' "$out"
printf '%s\n' 'ok process-runtime denied-before-start'
