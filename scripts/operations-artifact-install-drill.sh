...
uid=$(id -u gradle); unnamed_gid=42424
chown -R "$uid:$(id -g gradle)" "$out"
...
install_scope() {
 ...
 if [ "$scope" = system ]; then
  ...
  runuser -u gradle -- scripts/install-artifacts.sh --scope "$scope" --manifest "$manifest" --source "$source" --root "$dest"
 fi
...
[ "$(stat -c %u:%g:%a "$out/rootless/bin/lkjmc")" = "$uid:$(id -g gradle):750" ]
changed=$out/changed
...
chown -R "$uid:$(id -g gradle)" "$changed"
old=$(sha256sum "$out/user/bin/lkjmc")
if runuser -u gradle -- env LKJMC_SOURCE_COMMIT="$LKJMC_SOURCE_COMMIT" LKJMC_INSTALL_FAULT=status \
...
runuser -u gradle -- env LKJMC_SOURCE_COMMIT="$LKJMC_SOURCE_COMMIT" scripts/install-artifacts.sh \
