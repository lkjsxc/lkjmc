...
instance log identity at its configured path plus its configured listener. Before
`probe.wait`, apply reads its configured port and finishes all current state and
step work, then releases its pooled PostgreSQL connection for the entire wait.
Daemon apply code must match every `BootstrapEffect` variant exhaustively; adding
a variant without a real adapter is a compile-time review blocker, not a catch-all
success path.
