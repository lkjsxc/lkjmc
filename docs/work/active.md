...
Exact release `b6d22115f1726aeb570e91900cabcc008ca55689` now serves the five-route local Paper/Folia menu, six-domain sync boundary, migration 53, scoped heartbeat, and truthful backend readiness. Exact jar identity, systemd restart, full Incus restart, three protocol pings, public proxy reachability, and private port boundaries are live-observed. No authorized online-mode player has exercised `/menu`, `/docs`, `/lkjmc` completion/status, or transfer, so player acceptance remains blocked. The next executable source step is replacing the inherited checkout-based installer with the smallest immutable install/update path matching this deployment.
...
- Menu implementation commit and current product release: `b6d22115f1726aeb570e91900cabcc008ca55689`; branch `main` is 30 commits ahead of `origin/main` before this post-deployment ledger update.
...
Commit this post-deployment ledger, then inspect `scripts/install.sh`, `scripts/install-artifacts.sh`, and the exact `b6d2211` deployment evidence to implement the smallest immutable install/update entrypoint without broadening runtime claims. When an authorized account is available, exercise `/lkjmc` completion/status/success/failure transfer and `/menu` docs navigation. Continue monitoring exact `b6d2211`; rollback migration 53 only through the named snapshot or verified database restore.
