...
The exact `2a250f0b27f3f6680249a16be33f4dfa06e6ed16` release serves the single-host network and live Velocity logs prove `/lkjmc` registration. Source commit `078aa1ae297f599cbbff75b4efc1cb9f9a510c4e` implements scoped heartbeat, has no independent-review source blockers, and passes the complete Rust/JVM/PostgreSQL/contract/containment gate. The current gate is an exact release, migration 52, three credentials, and live freshness/staleness/reconnect evidence. Real-client command/completion/transfer remains blocked on an authorized online-mode account.
...
- Obsolete inventory deletion commit: `ba880aefca5e9eec05c00788adc754db8fb03ebd`; heartbeat source commit: `078aa1ae297f599cbbff75b4efc1cb9f9a510c4e`. Branch `main` tracks `origin/main` and is 22 commits ahead before this ledger checkpoint.
- Final source review is `tmp/agent/heartbeat-review-final.md`; complete disposable-PostgreSQL gate evidence for exact source commit `078aa1a` is `tmp/agent/heartbeat-20260814T083113Z/` via `tmp/agent/heartbeat-latest`.
...
- Heartbeat endpoint/reporter, exact credential policy and route verification, environment isolation, fresh/stale availability, proxy-registration rollback, request/database deadlines, migration-51 upgrade, fresh-schema source tests, independent source review, and complete disposable-PostgreSQL verification: committed in `078aa1a`; not yet released, migrated, credentialed, or live-observed.
...
- The serving release still has no plugin heartbeat; status therefore remains `ready:null`/non-joinable until `078aa1a` is released, migrated, credentialed, and observed.
...
Commit this ledger checkpoint and build a clean exact release. Verify the existing backup and create a pre-update snapshot, apply migration 52, create distinct `proxy`, `hub`, and `survival` credentials without exposing their values, deploy through the rollback-capable release path, and prove fresh/stale status plus systemd and container restart reconnect. Real command/completion/transfer acceptance remains blocked until an authorized online-mode client is available.
