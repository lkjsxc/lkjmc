...
| Typed profile and durable workflow intent | [player profile](../product/sync/player-profile.md); [transfer safety](../product/sync/transfer-safety.md) | `crates/lkjmc-core/src/profile_validation.rs`; `crates/lkjmc-store/src/data_workflows`; `migrations/045-durable-data-workflows.sql` | `scripts/check-data-workflows.py`; `crates/lkjmc-store/tests/data_workflows.rs` | none | PostgreSQL intent and failure facts do not prove player, inventory, runtime, or cleanup effects; Java bridges remain absent. | `A-SYNC`, `A-RUNTIME` |
