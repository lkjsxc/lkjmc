...
| --- | --- | --- | --- | --- | --- | --- |
| Document-driven Paper menu and hotbar token | [GUI](../product/gui/README.md) | `platforms/jvm/common/src/main/java/com/lkjmc/common/menu`; `platforms/jvm/paper/src/main/java/com/lkjmc/paper/PaperMenuAdapter.java`; `platforms/jvm/common/src/generated/resources/lkjmc-menu-bundle.json` | `:platforms:jvm:paper:menuProbes`; `:platforms:jvm:paper:menuCheckerMutations`; `scripts/check-menus.py`; `scripts/check-jvm-containment.py --artifacts` | none | All 62 routes render current, stale, or unavailable views; mutations have no daemon port and deny without current capability plus attestation. Protocol evidence is not a live player. | guarded Minecraft lane |
