...
| Atomic economy exchange and catalog purchases | [economy](../product/economy/README.md) | `crates/lkjmc-store/src/{points,exchange,shop}.rs`; `platforms/jvm/paper/src/main/java/com/lkjmc/paper/{ExchangeCommandAdapter,ShopCommandAdapter}.java` | Economy fault probes and PostgreSQL-gated store tests | `LKJMC_PLAYABLE_SMOKE=1 LKJMC_ACCEPT_MINECRAFT_EULA=1 ./scripts/check-playable-smoke.sh` | Ambiguous client inventory mutation is contained for operator reconciliation; live delivery remains guarded. | `F-SAFE-ECON` |
