...
| --- | --- | --- | --- | --- | --- | --- |
| Typed Java menu presentation | [commands](../product/commands/minecraft.md) | `platforms/jvm/paper/src/main/java/com/lkjmc/paper/PaperMenuAdapter.java`; `platforms/jvm/paper/src/main/java/com/lkjmc/paper/PaperMenuProtocolAdapter.java`; `platforms/jvm/velocity/src/main/java/com/lkjmc/velocity/VelocityMotdAdapter.java` | seven `menuProbes`; `scripts/check-jvm-containment.py --artifacts` | none | Paper registers only `/menu` and `/docs`; typed menu/permission/claim/settings views are read-only and mutation denial is final without attestation and a typed port. Velocity has MOTD/tab-list only. | guarded Minecraft lane |
