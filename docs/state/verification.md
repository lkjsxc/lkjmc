...
| Reproducible operations lab | [clean-room lab](../operations/clean-room-lab.md) | `scripts/run-operations-lab.py`; `scripts/check-operations.py`; `scripts/backup-postgres.sh`; `scripts/artifact-manifest.py`; `.github/workflows/verify.yml` | eight exact probes from `scripts/check-operations.py`; full lab writes private evidence only after every lane passes | none | Rootless engine, signing identity, production cutover, external routing, and capacity remain separate prerequisites. | `A-CUTOVER` |
