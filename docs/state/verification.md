...
| Fast and full local verification tiers | [CI](../operations/continuous-integration.md) | `scripts/verify-fast.sh`; `scripts/verify-full.sh`; `docker-compose.yml` | `./scripts/verify-fast.sh`; `./scripts/verify-full.sh`; `docker compose --profile verify run --rm verify` | none | Full output identifies every executed smoke and nested skip. Compose runs claim and web checks but remains environment-backed deterministic verification, not external live proof. | `F-SAFE-OPS` |
...
| Test-only seeded fault replay | [verification](../operations/verification.md) | `crates/lkjmc-daemon/src/fault_harness`; `scripts/check-fault-harness.py` | `./scripts/check-fault-harness.py` | none | The JSON evidence is a bounded injected-failure record with pending quality review; it is not release or product proof. | `F-FAULTS` |
