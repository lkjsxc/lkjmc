...
| Bounded local observability and private support collection | [observability](../architecture/runtime/daemon/observability.md); [support bundles](../operations/support-bundles.md) | `crates/lkjmc-core/src/observability/`; `crates/lkjmc-store/src/observability/`; `crates/lkjmc-daemon/src/observability/`; `crates/lkjmc-daemon/src/support/bundle/`; `migrations/050-observability.sql` | `scripts/check-observability.py` (six named probes and mutations) | none | Authenticated bounded queries, metrics, readiness, 30-repeat HTTP/PostgreSQL correlation, and private redacted archives prove daemon-local facts only. | No independently attested observer or external live signal is claimed. | `B-O` |
