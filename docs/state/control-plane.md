...
| Fenced per-instance runtime lifecycle | [adapters](../architecture/runtime/adapters.md); [desired state](../architecture/orchestration/desired-state.md) | `crates/lkjmc-daemon/src/runtime/`; `crates/lkjmc-store/src/runtime_adoption/`; `migrations/046-runtime-adoption.sql` | `scripts/check-runtime-adoption.py` (seven named probes) | none | Local effects use keyed serialization, durable fence ownership, identity-bound processes, bounded recovery, and append-only attempts. Kubernetes observation is diagnostic; start, stop, and delete reject before mutation because mounted launch inputs and durable race-safe object ownership are incomplete. | Kubernetes lifecycle and live proof remain unavailable until mounted inputs and UID/resourceVersion-preconditioned ownership reach the adapter. | `A-RUNTIME` |
| Durable imperative network intent and fail-closed adoption | [desired network](../architecture/bootstrap/desired-network.md) | `crates/lkjmc-core/src/network_intent.rs`; `crates/lkjmc-daemon/src/commands/bootstrap_api/`; `crates/lkjmc-store/src/network_intent.rs`; `migrations/048-network-intent.sql`; `migrations/049-network-attempt-recovery.sql` | `scripts/check-network-adoption.py` (six named probes) | none | PostgreSQL records correlated intent and append-only attempts. Local integration proves bounded real filesystem and child-process effects; examples omit unacquired artifacts and therefore deny before effects. Kubernetes apply remains unsupported. | Real Minecraft artifacts and guarded player readiness are not claimed until immutable inputs are acquired. | `A-NETWORK` |
| Bounded local observability and private support collection | [observability](../architecture/runtime/daemon/observability.md); [support bundles](../operations/support-bundles.md) | `crates/lkjmc-core/src/observability/`; `crates/lkjmc-store/src/observability/`; `crates/lkjmc-daemon/src/observability/`; `crates/lkjmc-daemon/src/support/bundle/`; `migrations/050-observability.sql`; `migrations/051-observability-attempt-identity.sql` | `scripts/check-observability.py` (six named probes and mutations) | none | Authenticated bounded queries, metrics, readiness, distinct per-execution events under reused request IDs, 30-repeat HTTP/PostgreSQL correlation, and seven-second private redacted archives prove daemon-local facts only. | No independently attested observer or external live signal is claimed. | `B-O` |
