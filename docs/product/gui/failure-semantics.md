...
- Session mismatch: the menu is stale.
- Render epoch mismatch: refresh the menu.
- Route mismatch: reopen from the current menu path.
...
reopens with the prior truthful state. If a purchase-like future action cannot
commit atomically, it must not be registered as live.
